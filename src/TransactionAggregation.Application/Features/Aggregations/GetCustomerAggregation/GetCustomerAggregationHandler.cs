    ...
    CancellationToken cancellationToken)
        {
            ...
            logger.LogInformation(
                "Aggregation requested for {CustomerId} from {From} to {To} (ForceRefresh={ForceRefresh})",
                request.CustomerId,
                request.From,
                request.To,
            ...
            // Simulate expensive multi-source aggregation work (joins, categorization, rollups).
            if (options.Value.ExpensiveComputationDelayMs > 0)
            {
                logger.LogDebug(
                    "Applying simulated expensive computation delay of {DelayMs}ms",
                    options.Value.ExpensiveComputationDelayMs);
                await Task.Delay(options.Value.ExpensiveComputationDelayMs, cancellationToken);
            }
